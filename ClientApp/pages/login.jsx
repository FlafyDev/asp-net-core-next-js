...
const Login = (props) => {
  ...
  const passwordRegex = new RegExp(String.raw`^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,}$`)
  const validateData = () => {
  ...
  return (
    <div className={[styles.container, (!loading || styles.loading)].join(' ')}>
      <div className={styles.panelContainer}>
        ...
        {/* Register Stage 0 */}
        <StyledInput visible={registerStage === 0} type="text"
          placeholder={"Username"} icon={faUser} state={[username, setUsername]} />

        <StyledInput visible={isRegistering && registerStage === 0} type="text"
          placeholder={"Email"} icon={faAt} state={[email, setEmail]} />

        <StyledInput visible={registerStage === 0} type="password"
          placeholder={"Password"} icon={faKey} state={[password, setPassword]} />

        {/* Register Stage 1 */}
        <div style={{ display: 'flex', flexDirection: 'row', width: '100%' }}>
          <StyledInput visible={registerStage === 1} type="text"
            placeholder={"First Name"} icon={faUser} state={[firstName, setFirstName]} />
          <div style={{ width: '50px' }}></div>
          <StyledInput visible={registerStage === 1} type="text"
            placeholder={"Last Name"} icon={faUser} state={[lastName, setLastName]} />
        </div>

        <StyledInput visible={registerStage === 1} type="number"
          placeholder={"Age"} icon={faCake} state={[age, setAge]}
          inputProps={{ min: 0, max: 99, value: '' }} />

        <div hidden={!(registerStage === 1)} className={styles.genderContainer}>
          {
            genderNames.map((genderName, i) => <div className={styles.genderSelector} onClick={() => setGender(i)} key={i}>
              <input type="radio" checked={gender === i} onChange={() => { }}></input>
        ...
        <div className={styles.bottomContainer}>
          <div className={styles.redirectContainer}>
            <a className={styles.redirect} onClick={changeMode}>{
              isRegistering ? (registerStage !== 0 ? (registerStage === maxRegisterStages ? "" : "Back") : "Login instead") : "Register instead"
            }</a>
          </div>
          <div className={styles.loginContainer}>
            <button className={styles.login} onClick={buttonOnClick}>{
              isRegistering ? (registerStage < maxRegisterStages - 1 ? "Next" : "Register") : "Login"
            }</button>
          </div>
        </div>
